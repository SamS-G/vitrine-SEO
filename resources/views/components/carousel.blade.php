        ...
        @foreach ($reviews as $index => $review)
            <div class="carousel-item {{ $index === 0 ? 'active' : '' }}">
                <div class="testimonial text-center p-4 bg-light rounded shadow">
                    <img src="{{ $review['profile_photo_url'] ?? 'https://via.placeholder.com/80' }}" alt="Photo de {{ $review['author_name'] }}" class="rounded-circle mb-3" width="80">
                    ...
                    <div class="stars">
                        ...
                        @endphp
                        @for ($i = 1; $i <= 5; $i++)
                            <span class="star {{ $i <= $rating ? 'text-warning' : 'text-secondary' }}">&#9733;</span>
                        @endfor
                    </div>
                </div>
            </div>
